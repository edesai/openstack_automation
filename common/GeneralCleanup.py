...
class GeneralCleanup(object):
    ...
    def start(self):
        ...
        if skip_nova is False and skip_proj is False:        
            ...
            '''
            try:
                instances = self.controller.listInstances(new_project.id, "admin", "cisco123")
                return
                for instance in instances:
                    self.controller.deleteInstance(new_project.id, self.new_user, self.new_password, instance.name)           
            except Exception as e:
                print "Error:", e
            
            try:
                networks = self.controller.listNetworks(new_project.id, self.new_user, self.new_password)
                for network in networks:
                    self.controller.deleteNetwork(network['id'], self.new_tenant, 
