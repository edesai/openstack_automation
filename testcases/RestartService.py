        ...
        try:
            ...
            for inst in range(self.num_inst):
                ...
                instObj = Instance(
                    ip=str(
                        (host1[inst].networks[
                            self.new_network1])[0]), instname=str(
                        host1[inst].name), hostname=hosts_list[0].hostname)
                ...
                instObj = Instance(
                    ip=str(
                        (host2[inst].networks[
                            self.new_network2])[0]), instname=str(
                        host2[inst].name), hostname=hosts_list[1].hostname)
            ...
            for inst in inst_ip_list:
                ...
                OvsFlowsCli().verify_ovs_and_vdp(
                    self.config_dict,
                    self.controller.ip,
                    self.controller.sys_username,
                    self.controller.password,
            ...
            if service == "enabler-server":
                ...
                enabler_inst = EnablerService(
                    ...
                    self.controller.sys_username,
                    self.controller.password)
                result = EnablerService.take_action(
                    enabler_inst, "restart", "server")
                if not result:
